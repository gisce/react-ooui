...
class MockConnectionProvider implements ConnectionProvider {
  ...
  async fieldsViewGet({ model, viewId, viewType, context }: any) {
    console.log("🔍 fieldsViewGet called:", { model, viewId, viewType, context });

    if (model === "sale.order" && viewType === "form") {
      console.log("🔍 Returning form view");
      console.log("🔍 Form view arch:", mockFormView.arch);
      console.log("🔍 Form view fields:", JSON.stringify(mockFormView.fields, null, 2));
      return mockFormView;
    }
    ...
    if (model === "sale.order.line" && viewType === "tree") {
      ...
      console.log("🔍 Tree view fields:", Object.keys(mockOne2ManyTreeView.fields));
      console.log("🔍 Full tree view:", JSON.stringify(mockOne2ManyTreeView, null, 2));
  ...
  async readObjects(params: {
    ...
    if (params.model === "sale.order.line") {
      console.log("🔍 FORCED: readObjects for sale.order.line, calling read directly");
  ...
  async read({ model, ids, fields }: any) {
    ...
    try {
      if (model === "sale.order" && ids.includes(1)) {
        ...
        const amount_untaxed = subtotals.reduce((sum, subtotal) => sum + subtotal, 0);
        ...
        const one2manyValue = {
          ...
          items: lines.map(line => {
            const processedLine = {
              ...
              product_id: Array.isArray(line.product_id) ? line.product_id : [line.product_id, line.product_id_name || ''],
              // Ensure order_id is properly formatted
              order_id: Array.isArray(line.order_id) ? line.order_id : [line.order_id, `SO/2024/000${line.order_id}`],
            };
        ...
        console.log("📋 Order line structured value being returned:", one2manyValue.items.length, "records");
        console.log("📋 All item IDs:", one2manyValue.items.map(item => item.id));
        console.log("📋 First few items with treeValues:", JSON.stringify(one2manyValue.items.slice(0, 2), null, 2));
        console.log("📋 Sample line data:", JSON.stringify(lines[0], null, 2));

        console.log("Returning sales order data:", JSON.stringify(result, null, 2));
        return result;
      }
    } catch (error) {
    ...
    if (model === "sale.order.line") {
      ...
      console.log("🔍 Found", lines.length, "order lines:", JSON.stringify(lines.slice(0, 2), null, 2));
      return lines;
    }
  ...
  async searchForTree({ model, domain, fields, limit = 100, offset = 0, order, context }: any) {
    console.log("🔍🔍🔍 searchForTree called:", { model, domain, fields, limit, offset, order, context });
    ...
    try {
      if (model === "sale.order.line") {
        ...
        if (domain && domain.length > 0) {
          ...
          for (let i = 0; i < flatDomain.length; i += 3) {
            ...
            if (field === "id" && operator === "in" && Array.isArray(value)) {
              ...
              filteredLines = filteredLines.filter(line => value.includes(line.id));
        ...
        console.log(`Returning ${paginatedLines.length} order lines (${offset}-${offset + limit} of ${finalLines.length})`);
        console.log("Sample paginated line:", JSON.stringify(paginatedLines[0], null, 2));
  ...
  async search({ model, domain, offset, limit, order, context, params, fieldsToRetrieve }: any) {
    console.log("🔍 search called with params:", { model, domain, offset, limit, order, params, fieldsToRetrieve });

    if (model === "sale.order.line") {
      console.log("🔍 Search called for sale.order.line - returning IDs directly");
  ...
  async unlink({ model, ids }: any) {
    ...
    if (model === "sale.order.line") {
      this.orderLineData = this.orderLineData.filter(line => !ids.includes(line.id));
      return true;
    }
  ...
  async fields_view_get({ model, view_id, view_type, context, toolbar, submenu }: any) {
    console.log("fields_view_get called:", { model, view_id, view_type, context });
    return this.fieldsViewGet({ model, viewId: view_id, viewType: view_type, context });
  }
  ...
  async load_views({ model, views, context }: any) {
    ...
    for (const [view_id, view_type] of views) {
      ...
      try {
        const view = await this.fieldsViewGet({ model, viewId: view_id, viewType: view_type, context });
        result[view_type] = view;
      } catch (error) {
        console.error(`Failed to load view ${view_type} for model ${model}:`, error);
      }
    }
  ...
  async executeOnChange({ model, action, ids, payload, fields }: any) {
    console.log("executeOnChange called:", { model, action, ids, payload, fields });
  ...
  async search_read({ model, domain, fields, limit, offset, order, context }: any) {
    console.log("🔍🔍🔍 search_read called:", { model, domain, fields, limit, offset, order, context });
    
    if (model === "sale.order.line") {
      // Use the same logic as searchForTree
      const result = await this.searchForTree({ model, domain, fields, limit, offset, order, context });
      return result.items || [];
    }
  ...
  async searchRead({ model, domain, fields, limit, offset, order, context }: any) {
    console.log("🔍🔍🔍 searchRead called:", { model, domain, fields, limit, offset, order, context });
    return this.search_read({ model, domain, fields, limit, offset, order, context });
  }

  // Other required methods with default implementations
  async fieldsGet() { return {}; }
  async getDefaults() { return {}; }
  async getToolbar() { return {}; }
  ...
  async readGroup({ model, domain, fields, groupby, offset, limit, order, context }: any) { 
  ...
  async saveUserViewPrefs({ key, preferences }: { key: string; preferences: any }) {
    console.log("🔍🔍🔍 saveUserViewPrefs called with key:", key, "preferences:", preferences);
      ...
      try {
        ...
        Object.keys(item).forEach((key) => {
          if (key === "id") {
            ...
            console.log(`🔍🔍🔍 Processing field ${key}, widget:`, widget?.constructor.name, widget?.type);
  ...
  async readEvalUiObjects({ model, ids, arch, fields, context, attrs }: any) {
    console.log("🔍🔍🔍 readEvalUiObjects called (THIS IS THE KEY METHOD!):", { model, ids, arch, fields, context, attrs });
    console.log("🔍🔍🔍 Requested IDs:", ids);
    console.log("🔍🔍🔍 Available order line IDs:", this.orderLineData.map(l => l.id));
    
    try {
      if (model === "sale.order.line") {
        // This is the main method that One2manyTree uses to fetch data
        const lines = this.orderLineData.filter(line => ids.includes(line.id));
        console.log("🔍🔍🔍 Found", lines.length, "matching lines for IDs:", ids);
        ...
        console.log("🔍🔍🔍 About to create TreeOoui with fields:", Object.keys(fields));
        ...
        console.log("🔍🔍🔍 readEvalUiObjects returning", tableItems.length, "records");
        console.log("🔍🔍🔍 Sample processed record:", JSON.stringify(tableItems[0], null, 2));
  ...
  async searchAllIds({ model, params, context, order }: any) {
    console.log("🔍🔍🔍 searchAllIds called:", { model, params, context, order });
    ...
    if (model === "sale.order.line") {
      ...
      console.log("🔍 searchAllIds returning", ids.length, "IDs:", ids.slice(0, 10));
...
export function initializeMockProvider() {
  ...
  mockProviderInstance = new Proxy(provider, {
    get(target, prop, receiver) {
      ...
      if (value === undefined && typeof prop === 'string' && prop !== 'constructor') {
...
export function getMockProvider(): MockConnectionProvider {
  if (!mockProviderInstance) {
    throw new Error("Mock provider not initialized. Call initializeMockProvider first.");
  }
  return mockProviderInstance;
}
